...
def build_default_profile(sensor_id: str, origin: str, destination: str, min_temp: float, max_temp: float) -> SensorContext:
    ...
    destination_coords = CITY_COORDS.get(destination) or CITY_COORDS.get(origin)
    return SensorContext(
        ...
        lat=profile.get("lat", destination_coords["lat"] if destination_coords else None),
        lng=profile.get("lng", destination_coords["lng"] if destination_coords else None),
...
async def sensor_worker(sensor: SensorContext, producer: AIOKafkaProducer):
    logger.info("[%s] Worker started", sensor.sensor_id)
